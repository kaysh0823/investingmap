/**
 * Phase 5G targeted browser QA only — does NOT run the full relation-browser matrix.
 */
...
const PORT_PREF = Number(process.env.RN_TEST_PORT || 8777);
let PORT = PORT_PREF;
let BASE = `http://127.0.0.1:${PORT}`;

const CASES = [
  { id: 'medtech/desktop/ko', path: '/medtech/korea_medtech_map.html', vp: { w: 1440, h: 900 }, lang: 'ko', ticker: '096530' },
  { id: 'medtech/tablet/en', path: '/medtech/korea_medtech_map.html', vp: { w: 768, h: 1024 }, lang: 'en', ticker: '328130' },
  { id: 'medtech/mobile/ko', path: '/medtech/korea_medtech_map.html', vp: { w: 375, h: 812 }, lang: 'ko', ticker: '145720' },
  { id: 'medtech/mobile/en', path: '/medtech/korea_medtech_map.html', vp: { w: 375, h: 812 }, lang: 'en', ticker: '060280' },
...
async function runCase(browser, c) {
  ...
  try {
    ...
    if (ready.usingLegacy && c.id.startsWith('medtech/')) failures.push('legacyFallback unexpectedly true');
    if (c.ticker && ready.selectedTicker !== c.ticker) {
      failures.push(`ticker expected ${c.ticker} got ${ready.selectedTicker}`);
    }
    if (c.id.startsWith('medtech/') && ready.model !== 'medical_device_product_regulatory_ecosystem') {
      failures.push(`medtech model: ${ready.model}`);
    }
...
async function main() {
  ...
  const out = {
    ...
    note: 'Phase 5G targeted only — full matrix not run; pageCreate hang = infrastructure failure without retry-as-success',
    results,
    appFailureCount: appFails.length,
    infrastructureFailureCount: infraFails.length,
  };
  const outPath = path.join(ROOT, 'docs/reports/phase5g-targeted-browser-qa.json');
