/**
 * Phase 5H targeted browser QA — full matrix NOT run.
 */
...
let PORT = Number(process.env.RN_TEST_PORT || 8778);
let BASE = `http://127.0.0.1:${PORT}`;

const CASES = [
  { id: 'software/desktop/ko', path: '/software/korea_software_map.html', vp: { w: 1440, h: 900 }, lang: 'ko', ticker: '035420', model: 'software_product_platform_ecosystem' },
  { id: 'software/tablet/en', path: '/software/korea_software_map.html', vp: { w: 768, h: 1024 }, lang: 'en', ticker: '018260', model: 'software_product_platform_ecosystem' },
  { id: 'software/mobile/ko', path: '/software/korea_software_map.html', vp: { w: 375, h: 812 }, lang: 'ko', ticker: '053800', model: 'software_product_platform_ecosystem' },
  { id: 'software/mobile/en', path: '/software/korea_software_map.html', vp: { w: 375, h: 812 }, lang: 'en', ticker: '042000', model: 'software_product_platform_ecosystem' },
  { id: 'telecom/desktop/ko', path: '/telecom/korea_telecom_map.html', vp: { w: 1440, h: 900 }, lang: 'ko', ticker: '017670', model: 'telecommunications_network_service_ecosystem' },
  { id: 'telecom/tablet/en', path: '/telecom/korea_telecom_map.html', vp: { w: 768, h: 1024 }, lang: 'en', ticker: '030200', model: 'telecommunications_network_service_ecosystem' },
  { id: 'telecom/mobile/ko', path: '/telecom/korea_telecom_map.html', vp: { w: 375, h: 812 }, lang: 'ko', ticker: '218410', model: 'telecommunications_network_service_ecosystem' },
  { id: 'telecom/mobile/en', path: '/telecom/korea_telecom_map.html', vp: { w: 375, h: 812 }, lang: 'en', ticker: '010170', model: 'telecommunications_network_service_ecosystem' },
...
async function runCase(browser, c) {
  ...
  try {
    ...
    if (ready.usingLegacy && (c.id.startsWith('software/') || c.id.startsWith('telecom/'))) failures.push('legacyFallback unexpectedly true');
    ...
    if (c.id.startsWith('software/') || c.id.startsWith('telecom/')) {
      stage = 'langReload';
...
async function main() {
  ...
  const out = {
    ...
    note: 'Phase 5H targeted only — full matrix not run; pageCreate hang = infrastructure without retry-as-success',
    results, appFailureCount: appFails.length,
    infrastructureFailureCount: results.filter((r) => r.infrastructureFailure).length,
  };
  const outPath = path.join(ROOT, 'docs/reports/phase5h-targeted-browser-qa.json');
