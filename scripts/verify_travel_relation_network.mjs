/**
 * verify:travel — travel, leisure & airlines value chain network
 */
import fs from 'fs';
...
import { FORBIDDEN_GENERIC_TRAVEL_IDS } from '../lib/relation_network/travel_network_canonical.mjs';

const ROOT = join(dirname(fileURLToPath(import.meta.url)), '..');
const failures = [];
const check = (cond, msg) => { if (!cond) failures.push(msg); };

const netFp = join(ROOT, 'data/networks/travel.json');
check(fs.existsSync(netFp), 'missing travel.json');
...
const profile = NETWORK_PROFILES.travel;
check(profile?.model === 'travel_leisure_airlines_value_chain_ecosystem', 'profile model');
check(profile?.layout === 'travelLeisureValueChainEcosystem', 'profile layout');
check(profile?.networkPath === '../data/networks/travel.json', 'profile networkPath');
check(network._legacyFallback === false, 'legacyFallback false');
check(!!network.travelCuratedAt, 'travel curated');
...
const invent = edges.filter((e) => ['operates_route', 'codeshare_with', 'operates_property'].includes(e.type)
...
check(peers.length >= 15, `peers (got ${peers.length})`);
check(peers.every((e) => e.defaultHidden === true), 'peers hidden');
const xref = edges.filter((e) => e.type === 'cross_sector_reference');
check(xref.length >= 2 && xref.every((e) => e.excludesFromOrphanResolution === true), 'xref ok');

const metrics = computeTravelMetrics(network);
...
for (const id of FORBIDDEN_GENERIC_TRAVEL_IDS) check(!byId.has(id), `forbidden ${id}`);
for (const [k, v] of Object.entries(metrics.claimCoverage || {})) {
  if (k === 'metricNotes' || !v || typeof v !== 'object') continue;
  const err = validateCoverageMetric(v, `travel.${k}`);
