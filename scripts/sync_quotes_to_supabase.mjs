...
async function verifyMomentumSchema(supabaseUrl, serviceKey) {
  const columns =
    'high_120d,low_120d,high_50d,low_50d,high_20d,low_20d,high_10d,low_10d,high_5d,low_5d,bb_upper,bb_lower';
  const url =
  ...
  throw new Error(
    `Supabase momentum schema unavailable (${res.status}). ` +
      `Apply supabase/migrations/0012_stock_quotes_momentum_bounds.sql, ` +
      `0016_stock_quotes_high_20d.sql, and 0019_stock_quotes_5d_10d.sql first. ` +
      body.slice(0, 180),
  );
}
...
async function upsertHistoryIndicatorsForTickers(tickers, rows, supabaseUrl, serviceKey) {
  ...
  let range10Attached = 0;
  let range5Attached = 0;
  let bbAttached = 0;
  for (const row of rows) {
    ...
    row.high_10d = bounds.high_10d;
    row.low_10d = bounds.low_10d;
    row.high_5d = bounds.high_5d;
    row.low_5d = bounds.low_5d;
    ...
    if (bounds.high_10d != null) range10Attached += 1;
    if (bounds.high_5d != null) range5Attached += 1;
  ...
  console.log(
    `  history indicators: spark20=${sparkAttached} range120=${range120Attached} ` +
      `range50=${range50Attached} range20=${range20Attached} range10=${range10Attached} ` +
      `range5=${range5Attached} bb50=${bbAttached}, upserted=${result.upserted.length}`,
  );
  return {
