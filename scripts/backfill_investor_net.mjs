...
async function main() {
  ...
  const existing = await fetchExistingInvestorNetDates(supabaseUrl, serviceKey);
  ...
  console.log(
    `Backfill investor net: target=${targetDays} trading days, ` +
  ...
  for (const dayYmd of missing.reverse()) {
    ...
    const result = await syncInvestorNetDay(dayYmd, supabaseUrl, serviceKey, INVESTOR_NET_CODES, env);
  ...
  const after = await fetchExistingInvestorNetDates(supabaseUrl, serviceKey);
  const elapsed = ((Date.now() - started) / 1000).toFixed(1);

  console.log('\n=== backfill_investor_net summary ===');
