...
function stripCssBlock(html) {
  ...
  if (start < 0) {
    // Fallback: #tab-graph / .rn-workspace rules without the banner comment
    return html.replace(/\n\s*#tab-graph\s*\{[\s\S]*?\}\s*/g, '\n').replace(/\n\s*\.rn-[^{]+\{[\s\S]*?\}\s*/g, '\n');
  }
  const endStyle = html.indexOf('</style>', start);
  if (endStyle < 0) return html;
  // Drop RN block only; keep trailing non-rn rules that sit before </style> if any.
  // Prefer cutting from marker to </style> (trust-footer dupes after RN are harmless to drop).
...
function stripGraphJsBlock(html) {
  ...
  // Include preceding separator lines of ═
  let cutStart = start;
  ...
  // Leave a stub so leftover svgEl references stay defined
  return html.slice(0, cutStart) + '\n\n    let svgEl = null;\n' + html.slice(end);
}
...
function stripRuntimeHooks(html) {
  let out = html;
  // i18n assign — remove bare null-deref lines
  out = out.replace(
  ...
  // switchTab graph branch
  out = out.replace(
    /\n\s*if\s*\(tab\s*===\s*['"]graph['"]\)\s*setTimeout\([^;]+;\s*\n\s*else if\s*\(window\.RelationNetwork\)\s*RelationNetwork\.onTabHidden\(\);?/g,
    '',
  );
  out = out.replace(
    /\n\s*if\s*\(tab\s*===\s*['"]graph['"]\)\s*setTimeout\([^;]+;/g,
    '',
  );
  out = out.replace(
    /\n\s*else if\s*\(window\.RelationNetwork\)\s*RelationNetwork\.onTabHidden\(\);?/g,
  ...
  // graph-hint assignments (optional)
  out = out.replace(
...
export function stripGraphFromHtml(html) {
  let out = html;
  out = stripCssBlock(out);
  out = stripGraphButton(out);
  out = stripGraphPanel(out);
  out = stripScriptTags(out);
  out = stripGraphJsBlock(out);
  out = stripI18nKeys(out);
  out = stripRuntimeHooks(out);
  // leftover whitespace tidy
  out = out.replace(/\n{3,}/g, '\n\n');
  return out;
}
...
function processFile(rel) {
  ...
  const after = stripGraphFromHtml(before);
  if (after !== before) fs.writeFileSync(fp, after, 'utf8');
  return { rel, before: Buffer.byteLength(before, 'utf8'), after: Buffer.byteLength(after, 'utf8'), changed: after !== before };
}
...
if (sample) {
  console.log(
    `  sample semiconductor HTML bytes: ${sample.before} → ${sample.after} (Δ ${sample.after - sample.before})`,
  );
}
