...
for (const rel of HTML_MAPS) {
  ...
  const next = html.replace(/live_quotes\.js\?v=\d+/g, 'live_quotes.js?v=19');
  if (next !== html) {
    fs.writeFileSync(fp, next, 'utf8');
    console.log('bumped live_quotes v=19', rel);
  }
}
...
if (fs.existsSync(bioHtml)) {
  ...
  if (next !== html) {
    ...
    console.log('bumped live_quotes v=19 bio/korea_bio_map.html');
  }
}
