...
export function quotePosition(last, low, high) {
  if (last == null || low == null || high == null || !Number.isFinite(last)) return null;
  if (high <= low) return null;
  return Math.round(((last - low) / (high - low)) * 1000) / 10;
}
...
export function buildSeoBodyBlock(geoKey, lastmod) {
  ...
  if (!copy) return '';
  return `  <section class="im-seo-body" id="im-seo-body" aria-labelledby="im-seo-body-title">
    <h2 id="im-seo-body-title" lang="ko">${escHtml(copy.titleKo)}</h2>
    <h2 id="im-seo-body-title-en" lang="en" hidden>${escHtml(copy.titleEn)}</h2>
    <p lang="ko" class="im-seo-body-p">${escHtml(copy.bodyKo)}</p>
    <p lang="en" class="im-seo-body-p" hidden>${escHtml(copy.bodyEn)}</p>
...
export function patchHreflang(html, pageUrl) {
  let out = html;
  out = out.replace(/<link rel="alternate" hreflang="ko[^"]*"[^>]*>/g, `<link rel="alternate" hreflang="ko-KR" href="${pageUrl}?lang=ko">`);
  out = out.replace(/<link rel="alternate" hreflang="en[^"]*"[^>]*>/g, `<link rel="alternate" hreflang="en-US" href="${pageUrl}?lang=en">`);
...
export const SEO_PRERENDER_CSS = `
    ...
    .im-seo-body h2 { font-size: 16px; color: var(--text); margin: 0 0 10px }
    .im-seo-body p { margin: 0 0 10px }
    .im-seo-body p[hidden], .im-seo-body h2[hidden] { display: none }
...
export function ensureSeoCss(html) {
  if (html.includes('.im-seo-body')) return html;
...
export function loadSnapshots(root) {
  ...
  return {
    ...
    rsMap,
    builtAt: quotesFile.builtAt || rsFile.builtAt || new Date().toISOString().slice(0, 10),
  };
}
...
export function prerenderMapPage({ root, rel, geoKey, base, pageMeta, snapshots }) {
  ...
  html = injectOrReplace(html, SEO_BODY_START, SEO_BODY_END, bodyBlock, '</section>\n\n  <div class="tabs">')
    || injectOrReplace(html, SEO_BODY_START, SEO_BODY_END, bodyBlock, '<div class="tabs">');
  if (!html.includes(SEO_BODY_START)) {
    html = html.replace(
      /(<section class="geo-summary map-editorial-collapsible"[\s\S]*?<\/section>)/,
      `$1\n${SEO_BODY_START}\n${bodyBlock}\n${SEO_BODY_END}`,
    );
  }
