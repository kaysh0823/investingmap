...
export function buildTableRows(companies, quotes, rsMap, globalMap) {
  ...
  return sorted
    .map((c) => {
      ...
      <td class="spark-cell">${sparkPlaceholder}</td>
      ...
      <td class="quote-cell ret-cell">${fmtReturnPct(rs.ret200dPct ?? rs.ret1yPct)}</td>
...
export function buildSeoBodyBlock(geoKey, lastmod) {
  ...
  const leadKo = copy.leadKo || parasKo[0] || '';
  const leadEn = copy.leadEn || parasEn[0] || '';
  const howToKo = copy.howToKo || '';
  const howToEn = copy.howToEn || '';
  // Detail = paragraphs after lead/howTo when those fields exist; else all but first.
  const hasLead = !!(copy.leadKo || copy.leadEn);
  const detailKo = hasLead
    ? parasKo.filter((p) => p !== leadKo && p !== howToKo)
    : parasKo.slice(1);
  const detailEn = hasLead
    ? parasEn.filter((p) => p !== leadEn && p !== howToEn)
    : parasEn.slice(1);
  const koLead = leadKo
    ? `    <p lang="ko" class="im-seo-lead map-editorial-lead">${escEditorialHtml(leadKo)}</p>\n`
    : '';
  const enLead = leadEn
    ? `    <p lang="en" class="im-seo-lead map-editorial-lead" hidden>${escEditorialHtml(leadEn)}</p>\n`
    : '';
  const koHow = howToKo
    ? `    <p lang="ko" class="im-seo-howto map-editorial-howto">${escEditorialHtml(howToKo)}</p>\n`
    : '';
  const enHow = howToEn
    ? `    <p lang="en" class="im-seo-howto map-editorial-howto" hidden>${escEditorialHtml(howToEn)}</p>\n`
    : '';
  const koDetail = detailKo
    .map((p) => `    <p lang="ko" class="im-seo-body-p im-seo-detail">${escEditorialHtml(p)}</p>`)
    .join('\n');
  const enDetail = detailEn
    .map((p) => `    <p lang="en" class="im-seo-body-p im-seo-detail" hidden>${escEditorialHtml(p)}</p>`)
    .join('\n');
...
${koLead}${enLead}${koHow}${enHow}    <div class="map-editorial-detail-wrap" id="map-editorial-detail-wrap">
    <button type="button" class="map-editorial-more-btn" id="map-editorial-more-btn" aria-expanded="false" aria-controls="map-editorial-detail" lang="ko">산업 해설·출처 보기</button>
    <button type="button" class="map-editorial-more-btn" id="map-editorial-more-btn-en" aria-expanded="false" aria-controls="map-editorial-detail" lang="en" hidden>Show industry notes &amp; sources</button>
...
${enDetail}
    ...
    <p class="im-seo-snapshot-note">표 시세·52주·RS 필드 스냅샷 기준일: ${escHtml(lastmod)} (영업일 /api/quotes로 갱신) · Table quote snapshot as of ${escHtml(lastmod)}. 시세 기준일과 콘텐츠 수정일은 다를 수 있습니다.</p>
    </div>
    </div>
  </div>
