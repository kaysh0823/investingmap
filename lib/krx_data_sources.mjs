/**
 * Resolves latest KRX CSVs under data/ by filename date (YMD), not mtime.
 * Cloudflare clean checkouts give identical mtimes; YMD keeps selection deterministic.
...
/** @returns {number} negative if a is newer than b (desc sort) */
function compareYmdDesc(a, b) {
...
/**
 * Sort CSV filenames newest-first by embedded YMD; unparseable names go last.
 * Equal YMD or both unparseable → mtime tie-break only.
 */
export function compareCsvFilenamesByDateDesc(a, b, dataDir) {
  const ya = extractYmdFromFilename(a);
  const yb = extractYmdFromFilename(b);
  if (ya && yb) {
    const cmp = compareYmdDesc(ya, yb);
    if (cmp !== 0) return cmp;
  } else if (ya && !yb) return -1;
  else if (!ya && yb) return 1;
  return fs.statSync(join(dataDir, b)).mtimeMs - fs.statSync(join(dataDir, a)).mtimeMs;
}
...
export function resolveLatestCsv(dataDir, prefix) {
  ...
  names.sort((a, b) => compareCsvFilenamesByDateDesc(a, b, dataDir));
...
export function loadListedEnglish3557Map(dataDir) {
  let p;
  try {
    p = resolveLatestCsv(dataDir, 'data_3557_');
  } catch {
    return new Map();
  }
  const text = readKrxCsvFile(p);
...
/**
 * Full listing meta from data_3557 (증권구분 / 소속부 / 주식종류).
 * Used by RS ordinary-share filter when apihub SECUGRP_NM is unavailable
 * (e.g. MDCSTAT01501 full-market history path).
 *
 * @returns {Map<string, {
 *   name: string,
 *   nameEn: string,
 ...
 * }>}
 */
export function loadListedShareMeta3557(dataDir) {
  let p;
  try {
    p = resolveLatestCsv(dataDir, 'data_3557_');
  } catch {
  ...
  for (const line of text.split(/\r?\n/).slice(1)) {
    ...
    const code = String(f[1] || '').trim().toUpperCase();
    if (!code) continue;
    const nameFull = String(f[2] || '').trim();
    const nameAbbr = String(f[3] || '').trim();
    const name = (nameAbbr || nameFull).replace(/^\(주\)/, '').trim();
    map.set(code, {
      name,
      nameEn: String(f[4] || '').trim(),
      market: normalizeMarketLabel(f[6]),
      secu: String(f[7] || '').trim(),
      dept: String(f[8] || '').trim(),
      kind: String(f[9] || '').trim(),
    });
  }
  return map;
}
