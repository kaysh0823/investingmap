...
export function buildCuratedSectorMap(config) {
  ...
  let companies = config.companies.map((seed, index) => {
    ...
    if (!row) {
      missingTickers.push(seed.ticker);
      console.warn(
        `${config.id}: KRX row missing for ${seed.ticker} — skipped (delisted?)`,
      );
      return null;
    }
    return {
  ...
  }).filter(Boolean).filter((c) => passesMcapFloor({ mcapWon: c.mcapWon }));
  if (missingTickers.length >= 2) {
    throw new Error(
      `${config.id}: KRX row missing for ${missingTickers.length} tickers `
        + `(${missingTickers.join(', ')}) — possible data file corruption`,
    );
  }
  ...
  if (isBigchip) {
    // bigchip has only 2 companies (Samsung, SK hynix); skip chain filter row and neutralize chain filter logic
    html = html.replace(
