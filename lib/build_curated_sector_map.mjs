...
function buildTranslations(config, companies) {
  ...
  const commonKo = {
    ...
    dataAsof: asof.ko,
    tabHeatmap: '🔥 시총 히트맵', heatmapHint: '시가총액 기준',
    tabTable: '📋 기업목록 & 필터', tabGraph: '🌐 관계 네트워크',
  ...
  const commonEn = {
    ...
    dataAsof: asof.en,
    tabHeatmap: '🔥 Market-cap heatmap', heatmapHint: 'By market cap',
    tabTable: '📋 Company list & filters', tabGraph: '🌐 Relationship network',
  ...
  return {
    ko: { title: `🇰🇷 ${config.titleKo}`, subtitle: config.subtitleKo, ...commonKo },
    en: { title: `🇰🇷 ${config.titleEn}`, subtitle: config.subtitleEn, ...commonEn },
  };
}
...
function globalsBlock(globals) {
  return '[\n' + globals.map((g) =>
    `      { id: '${esc(g.id)}', name: '${esc(g.name)}', nameEn: '${esc(g.nameEn || g.name)}', country: '${esc(g.country)}', region: '${esc(g.region)}', sector: '${esc(g.sector)}' }`
  ).join(',\n') + '\n    ]';
}
