...
export const SECTOR_EDITORIAL = {
  semi: {
    ko: {
      ...
      howTo: "사용 예: (1) 히트맵에서 시총·당일 등락을 훑고, (2) 기업 표에서 체인·PER·PBR·52주 위치를 정렬·필터하며, (3) 모멘텀·변동성·밸류에이션·퍼포먼스 캘린더 탭으로 상대 강도·변동·배수 분포를 확인하세요.",
      paragraphs: [
    ...
    en: {
      ...
      howTo: "How to use: (1) scan the heatmap for size and 1-day moves, (2) sort/filter the company table by chain, PER, PBR, and 52-week position, (3) open momentum, volatility, valuation, or performance-calendar tabs for relative strength, range, multiples, and YTD paths.",
      paragraphs: [
  ...
  robot: {
    ko: {
      ...
      howTo: "히트맵·표로 규모와 체인을 본 뒤, 모멘텀으로 상대 강도, 밸류에이션으로 배수 분포를 확인하세요. 설비투자·제조업 경기에 민감하므로 공시의 수주·고객 비중을 함께 봅니다.",
      paragraphs: [
    ...
    en: {
      ...
      howTo: "Scan heatmap and table by chain, then use momentum and valuation tabs. Capex and manufacturing cycles matter—check filings for orders and customer mix.",
      paragraphs: [
  ...
  battery: {
    ko: {
      ...
      howTo: "사용 예: 히트맵으로 시총·등락을 보고, 표에서 셀/소재/장비 등 체인별로 정렬한 뒤, 밸류에이션 탭에서 PER·PBR·배당 분포를, 모멘텀·변동성 탭에서 RS·고저폭을 확인하세요.",
      paragraphs: [
    ...
    en: {
      ...
      howTo: "Use the heatmap for size and daily moves, sort the table by chain, then check valuation for multiples and momentum/volatility for RS and range.",
      paragraphs: [
  ...
  bigchip: {
    ko: {
      ...
      howTo: "표·히트맵·모멘텀·밸류에이션 탭으로 두 종목과 관련 상장 노드의 지표를 비교하세요. 삼성전자·SK하이닉스는 일반 반도체 지도에 중복 편입하지 않습니다.",
      paragraphs: [
    ...
    en: {
      ...
      howTo: "Use the table, heatmap, momentum, and valuation tabs. Neither name is duplicated on the general semiconductor map.",
      paragraphs: [
