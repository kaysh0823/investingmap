...
export const SECTOR_EDITORIAL = {
  ...
  ship: {
    ko: {
      paragraphs: [
        '조선소, 엔진·기자재, 철강·용접, 해양플랜트, 해운, 방산 해양 등 조선/해운 밸류체인에 속한 상장사를 묶어 보여 줍니다. 수주 잔량·선종 mix는 각 기업 설명에 반영하되, 실시간 수주 공시는 별도로 확인해야 합니다.',
        '조선/해운은 LNG선·컨테이너선 사이클과 환율, 원자재 가격에 민감합니다. 표의 주가 위치는 52주 구간 대비 현재가를 백분율로 나타내며, 동종 업체 간 상대적 위치 비교에 유용합니다. 글로벌 조선·해운 peer와의 참고 관계는 그래프에서 회색·실선으로 구분됩니다.',
        '편집 분류 기준과 데이터 출처는 <a href="../editorial-policy.html">편집·검증 정책</a>에 정리되어 있습니다.',
      ],
    },
    en: {
      paragraphs: [
        'Listed companies across the shipbuilding, marine equipment, and shipping value chain—yards, engines and marine equipment, steel and welding, offshore, shipping, and defense-marine—are grouped on one map. Order backlog and vessel-mix themes are reflected in company notes; verify real-time order disclosures separately.',
        'Shipbuilding and shipping are sensitive to LNG and container-ship cycles, FX, and raw-material prices. The 52-week range column shows where the last price sits within the 52-week high/low as a percentage, useful for comparing peers. Reference relationships to global yard and shipping peers are shown as gray and solid lines in the graph.',
  ...
  bigchip: {
    ko: {
      paragraphs: [
        '삼성전자/하이닉스 지도는 삼성전자와 SK하이닉스를 중심으로 후방 공급사·글로벌 peer·전방 고객 관계를 한 화면에 둡니다. 관계 네트워크 좌측 밸류체인 범례는 국내 상장 노드를 반도체 섹터와 같은 분류(IDM/종합반도체, 팹리스, 파운드리, 소재, 전·후공정 장비, 부품/기판, 패키징/테스트, 유통)로 색·그룹합니다.',
        '종목·시장 구분과 시가총액·PER·PBR은 페이지 기준일의 KRX 데이터에서 가져오며, 기존 반도체 지도에서는 두 종목을 제외해 섹터별 단일 소속을 유지합니다. 메모리 업황은 DRAM·NAND 가격, AI 서버용 HBM 수요, 설비투자와 재고 사이클에 민감합니다.',
    ...
    en: {
      paragraphs: [
        'The Samsung/SK hynix map centers on Samsung Electronics and SK hynix with upstream suppliers, global peers, and downstream customers. The left-hand value-chain legend colors domestic listed nodes with the same semiconductor taxonomy used on the semi map (IDM, fabless, foundry, materials, front/back-end equipment, substrates, packaging & test, distribution).',
        'Listings, market classification, market cap, PER, and PBR follow KRX data as of the page date; both names are removed from the broader semiconductor map to preserve one-sector membership. Memory earnings are sensitive to DRAM and NAND pricing, AI-server HBM demand, capital spending, and inventory cycles.',
  ...
  chemical: {
    ko: {
      paragraphs: [
        '화학·정유 지도는 국내 상장사를 석유화학, 정밀·특수화학, 정유·가스, 화학소재로 나눠 비교합니다. 롯데케미칼·금호석유화학·S-Oil 등 KRX 시가총액 2천억원 이상 종목을 중심으로 PER·PBR과 밸류체인 분류를 한 화면에서 볼 수 있습니다.',
  ...
  travel: {
    ko: {
      paragraphs: [
        '여행·레저·항공 지도는 국내 상장사를 항공, 카지노, 호텔·리조트, 여행·면세로 나눠 비교합니다. 대한항공·강원랜드·호텔신라·하나투어 등 KRX 시가총액 2천억원 이상 종목의 PER·PBR과 밸류체인 분류를 한 화면에서 볼 수 있습니다.',
    ...
    en: {
      paragraphs: [
        'The travel, leisure and airlines map groups listed Korean companies into airlines, casino, hotels & resorts, and travel agencies/duty-free. It covers names above the KRW 200 billion market-cap floor such as Korean Air, Kangwon Land, Hotel Shilla, and Hanatour with PER, PBR, and value-chain tags on one screen.',
