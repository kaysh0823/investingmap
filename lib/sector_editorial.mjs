...
export const SECTOR_EDITORIAL = {
  ...
  bigchip: {
    ko: {
      paragraphs: [
        '반도체 대형주 지도는 삼성전자와 SK하이닉스만을 분리해 메모리, HBM, 시스템반도체와 파운드리 노출을 비교합니다. 종목·시장 구분과 시가총액·PER·PBR은 페이지 기준일의 KRX 데이터에서 가져오며, 기존 반도체 지도에서는 두 종목을 제외해 섹터별 단일 소속을 유지합니다.',
    ...
    en: {
      paragraphs: [
        'The chip-leaders map isolates Samsung Electronics and SK hynix for comparison across memory, HBM, system semiconductors, and foundry exposure. Listings, market classification, market cap, PER, and PBR follow KRX data as of the page date; both names are removed from the broader semiconductor map to preserve one-sector membership.',
