...
import { chainOverride } from './chain_overrides.mjs';
...
function englishOnlyFallback(industryKey) {
  const map = {
    ...
    battery: 'Battery products and services',
    renewable: 'Renewable energy products and services',
    nuclear: 'Nuclear power products and services',
    kculture: 'K-culture products and services',
    kconsume: 'K-consume, retail and travel products and services',
    cosmetics: 'Cosmetics and K-beauty products and services',
...
function pickOverride(overrides, ticker, industryKey) {
  ...
  if (!o) return null;
  const flat = o.semType || o.products || o.chain ? o : null;
  const byIndustry = o.byIndustry?.[industryKey];
  // byIndustry는 산업별 값(예: semi 전용 chain)만 담을 수 있으므로 flat 값 위에 덮어쓴다.
  if (byIndustry) return flat ? { ...flat, ...byIndustry } : byIndustry;
  return flat;
}
...
function productsFromSubSector(industryKey, sub, chain, name) {
  ...
  switch (industryKey) {
    ...
    case 'battery':
      if (eq(/ESS|연료전지|수소|PCS|전력변환|저장/i)) return ko(`${s} — ESS·에너지저장`, `${s} — ESS and energy storage`);
      if (eq(/배터리|2차전지|양극|음극|전해질|리튬/i)) return ko(`${s} — 2차전지·ESS`, `${s} — batteries and ESS`);
      return ko(`${s} — 2차전지 밸류체인`, englishOnlyFallback('battery'));

    case 'renewable':
      if (eq(/태양|솔라|solar|모듈|폴리실리콘/i)) return ko(`${s} — 태양광`, `${s} — solar PV`);
      if (eq(/풍력|wind|타워/i)) return ko(`${s} — 풍력`, `${s} — wind power`);
      if (eq(/수소|hydrogen/i)) return ko(`${s} — 수소`, `${s} — hydrogen`);
      return ko(`${s} — 신재생에너지`, englishOnlyFallback('renewable'));

    case 'nuclear':
      if (eq(/원자로|SMR|주기기|터빈/i)) return ko(`${s} — 원자로·주기기·SMR`, `${s} — reactors, major components and SMR`);
      if (eq(/설계|EPC/i)) return ko(`${s} — 원전 설계·EPC`, `${s} — nuclear design and EPC`);
      if (eq(/정비|운영|O&M/i)) return ko(`${s} — 원전 운영·정비`, `${s} — nuclear operations and maintenance`);
      return ko(`${s} — 원전 기자재`, englishOnlyFallback('nuclear'));
    ...
    case 'kconsume':
      ...
      return ko(`${s} — K-소비·유통`, englishOnlyFallback('kconsume'));

    case 'cosmetics':
      if (eq(/ODM|OEM|콜마|코스맥스|코스메카/i)) return ko(`${s} — 화장품 ODM·OEM`, `${s} — cosmetics ODM/OEM`);
      if (eq(/유통|채널|역직구|실리콘/i)) return ko(`${s} — 화장품 유통·채널`, `${s} — cosmetics channels`);
      if (eq(/원료|성분|ingredient/i)) return ko(`${s} — 화장품 원료`, `${s} — cosmetics ingredients`);
      if (eq(/용기|패키징|packaging/i)) return ko(`${s} — 화장품 용기`, `${s} — cosmetics packaging`);
      if (eq(/미용기기|HIFU|레이저|에스테틱|스킨부스터|리쥬란|클래시스|원텍|파마리서치/i))
        return ko(`${s} — 미용기기`, `${s} — aesthetic devices`);
      return ko(`${s} — 화장품 브랜드`, englishOnlyFallback('cosmetics'));
...
export function enrichCompanyFields(c, industryKey, cpMap, overrides) {
  ...
  // data/chain_overrides.json is the sole chain assignment source when present.
  const forcedChain = chainOverride(industryKey, c.ticker) || ov?.chain;
  if (forcedChain) c.chain = forcedChain;

  // Explicit ticker_field_overrides win over stale seeded copy (corrections, not fill-empty-only).
  if (ov?.semType) c.semType = ov.semType;
  else if (EMPTY(c.semType)) c.semType = sub || c.chain || '—';

  if (ov?.semTypeEn) c.semTypeEn = ov.semTypeEn;
  else if (EMPTY(c.semTypeEn)) {
    c.semTypeEn = HANGUL.test(c.semType || '') ? '—' : c.semType;
  ...
  if (Array.isArray(ov?.tags)) {
    for (const tag of ov.tags) {
      if (tag && !String(c.products || '').includes(tag)) {
        c.products = c.products ? `${c.products} · ${tag}` : tag;
      }
    }
  }

  if (ov?.name) c.name = ov.name;
  if (ov?.nameEn) c.nameEn = ov.nameEn;
