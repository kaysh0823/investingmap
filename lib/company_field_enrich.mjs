...
function productsFromSubSector(industryKey, sub, chain, name) {
  ...
  switch (industryKey) {
    case 'semi':
      ...
      return ko(`${s} — 반도체 관련 제품`, `${s} — ${s} related products`);
    ...
    case 'ship':
      ...
      return ko(`${s} — 조선·해양 관련`, `${s} — ${s} for shipbuilding and marine`);
    ...
    case 'defense':
      ...
      return ko(`${s} — 방위·항공우주`, `${s} — ${s} defense and aerospace`);
    ...
    case 'robot':
      ...
      return ko(`${s} — 로봇·자동화`, `${s} — ${s} robotics and automation`);
    ...
    case 'energy':
      ...
      return ko(`${s} — 에너지·전력`, `${s} — ${s} energy and power`);
    ...
    case 'kculture':
      ...
      if (eq(/게임/i)) return ko(`${s} — 게임·IP`, `${s} — games and IP`);
      ...
      return ko(`${s} — K-컬처`, `${s} — ${s} K-culture`);
    ...
    case 'bio':
      ...
      return ko(`${s} — 바이오·헬스케어`, `${s} — ${s} biotech and healthcare`);

    default:
      return ko(`${s} 관련 제품·서비스`, `${s} related products and services`);
  }
}
...
export function enrichCompanyFields(c, industryKey, cpMap, overrides) {
  ...
  if (EMPTY(c.semTypeEn)) {
    c.semTypeEn = ov?.semTypeEn || c.semType;
  }
  ...
  if (EMPTY(c.productsEn) && !EMPTY(c.products)) {
    c.productsEn = ov?.productsEn || c.products;
  }
