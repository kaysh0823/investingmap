...
export function validateNetworkReport(network) {
  ...
  if (sectorKey === 'software') {
    ...
    for (const n of nodes) {
      if (n.type === 'listed_company' && n.id && !String(n.id).startsWith('krx:')) {
        fail(`software listed_company id must be krx: ${n.id}`);
      }
      if ((n.type === 'software_product' || n.type === 'platform' || n.type === 'cloud_service'
        || n.type === 'software_category' || n.type === 'customer_industry')
        && String(n.id).startsWith('krx:')) {
        fail(`software product/platform must not use listed company id ${n.id}`);
      }
      if (FORBIDDEN.has(n.id)) fail(`software forbidden generic ID ${n.id}`);
      if (n.type === 'listed_company') {
        if (listedTickers.has(n.ticker)) fail(`software duplicate listed ticker ${n.ticker}`);
        listedTickers.add(n.ticker);
      }
    }
    for (const e of edges) {
      if (e.type === 'peer' && e.defaultHidden !== true) {
        fail(`software peer edge ${e.id} must be defaultHidden`);
      }
      if (['integrates_with', 'supports_environment', 'used_in_industry', 'exposed_to_market'].includes(e.type)
        && ['confirmed', 'reported'].includes(e.status)) {
        fail(`software ${e.id} integration/industry exposure must not be confirmed business`);
      }
      if (e.type === 'cross_sector_reference') {
        if (e.excludesFromBusinessCoverage !== true) {
          warn(`software cross_sector_reference ${e.id} should set excludesFromBusinessCoverage`);
        }
        if (e.status === 'confirmed' || e.status === 'reported') {
          fail(`software cross_sector_reference ${e.id} must not be confirmed business`);
        }
      }
      if ((e.type === 'owns' || e.type === 'owns_stake_in') && e.status === 'confirmed') {
        if (e.stakePct == null || !e.asOf) fail(`software confirmed ownership ${e.id} needs stakePct/asOf`);
      }
    }
    const metrics = network.metrics || {};
    const cc = metrics.claimCoverage || {};
    for (const key of Object.keys(cc)) {
      ...
      const val = cc[key];
      if (!val || typeof val !== 'object') continue;
      const err = validateCoverageMetric(val, `software.${key}`);
      if (err) fail(err);
    }
    if (metrics.confirmedBusinessEdgeCount > 15) {
      warn(`software confirmed business count ${metrics.confirmedBusinessEdgeCount} exceeds Phase 5H cap 15`);
    }
  }
  ...
  if (sectorKey === 'robot') {
    ...
    for (const n of nodes) {
      if (n.type === 'listed_company' && n.id && !String(n.id).startsWith('krx:')) {
        fail(`robot listed_company id must be krx: ${n.id}`);
      }
      if ((n.type === 'robot_product' || n.type === 'robot_category' || n.type === 'robot_component'
        || n.type === 'reducer' || n.type === 'actuator' || n.type === 'sensor'
        || n.type === 'controller' || n.type === 'robot_software' || n.type === 'application')
        && String(n.id).startsWith('krx:')) {
        fail(`robot product/component must not use listed company id ${n.id}`);
      }
      if (FORBIDDEN.has(n.id)) fail(`robot forbidden generic ID ${n.id}`);
      if (n.type === 'listed_company') {
        if (listedTickers.has(n.ticker)) fail(`robot duplicate listed ticker ${n.ticker}`);
        listedTickers.add(n.ticker);
      }
    }
    for (const e of edges) {
      if (e.type === 'peer' && e.defaultHidden !== true) {
        fail(`robot peer edge ${e.id} must be defaultHidden`);
      }
      if (['supports_application', 'used_in_end_market', 'member_of_category', 'produces_component', 'produces_robot', 'develops', 'uses_component'].includes(e.type)
        && ['confirmed', 'reported'].includes(e.status)) {
        fail(`robot ${e.id} structure/application must not be confirmed business`);
      }
      if (e.type === 'cross_sector_reference') {
        if (e.excludesFromBusinessCoverage !== true) {
          warn(`robot cross_sector_reference ${e.id} should set excludesFromBusinessCoverage`);
        }
        if (e.status === 'confirmed' || e.status === 'reported') {
          fail(`robot cross_sector_reference ${e.id} must not be confirmed business`);
        }
      }
      if ((e.type === 'owns' || e.type === 'owns_stake_in') && e.status === 'confirmed') {
        if (e.stakePct == null || !e.asOf) fail(`robot confirmed ownership ${e.id} needs stakePct/asOf`);
      }
      if (['supplies_component_to', 'supplies_robot_to', 'deployed_at'].includes(e.type)
        && e.status === 'confirmed' && !(e.evidence || []).some((ev) => ev.directEvidence === true)) {
        fail(`robot confirmed supply/deploy ${e.id} needs directEvidence`);
      }
    }
    if (network._legacyFallback === true) fail('robot legacyFallback must be false');
    if (!network.phase5iCuratedAt) warn('robot missing phase5iCuratedAt');
    const metrics = network.metrics || {};
    const cc = metrics.claimCoverage || {};
    for (const key of Object.keys(cc)) {
      if (key === 'metricNotes') continue;
      const val = cc[key];
      if (!val || typeof val !== 'object') continue;
      const err = validateCoverageMetric(val, `robot.${key}`);
      if (err) fail(err);
    }
    if (metrics.confirmedBusinessEdgeCount > 15) {
      warn(`robot confirmed business count ${metrics.confirmedBusinessEdgeCount} exceeds Phase 5I cap 15`);
    }
  }

  if (sectorKey === 'telecom') {
    const listedTickers = new Set();
    const FORBIDDEN = new Set([
    ...
    for (const n of nodes) {
      if (n.type === 'listed_company' && n.id && !String(n.id).startsWith('krx:')) {
        fail(`telecom listed_company id must be krx: ${n.id}`);
      }
      if (FORBIDDEN.has(n.id)) fail(`telecom forbidden generic ID ${n.id}`);
      if (n.type === 'license_or_allocation' && !n.identifier && !n.licenseIdentifier) {
        fail(`telecom license node ${n.id} missing identifier`);
      }
      if (n.type === 'listed_company') {
        if (listedTickers.has(n.ticker)) fail(`telecom duplicate listed ticker ${n.ticker}`);
        listedTickers.add(n.ticker);
      }
    }
    for (const e of edges) {
      if (e.type === 'peer' && e.defaultHidden !== true) {
        fail(`telecom peer edge ${e.id} must be defaultHidden`);
      }
      if (['assigned_by', 'licensed_by', 'uses_spectrum_band', 'authorized_for_service'].includes(e.type)
        && ['confirmed', 'reported'].includes(e.status)
        && (e.type === 'assigned_by' || e.type === 'licensed_by' || e.countsAsOwnership === true)) {
        fail(`telecom ${e.id} spectrum/license must not count as ownership business`);
      }
      if (['assigned_by', 'licensed_by'].includes(e.type) && ['confirmed', 'reported'].includes(e.status)) {
        fail(`telecom ${e.id} spectrum assignment must not be confirmed/reported business`);
      }
      if (e.type === 'cross_sector_reference') {
        if (e.excludesFromBusinessCoverage !== true) {
          warn(`telecom cross_sector_reference ${e.id} should set excludesFromBusinessCoverage`);
        }
        if (e.status === 'confirmed' || e.status === 'reported') {
          fail(`telecom cross_sector_reference ${e.id} must not be confirmed business`);
        }
      }
      if ((e.type === 'owns' || e.type === 'owns_stake_in') && e.status === 'confirmed') {
        if (e.stakePct == null || !e.asOf) fail(`telecom confirmed ownership ${e.id} needs stakePct/asOf`);
      }
    }
    const metrics = network.metrics || {};
    const cc = metrics.claimCoverage || {};
    for (const key of Object.keys(cc)) {
      ...
      const val = cc[key];
      if (!val || typeof val !== 'object') continue;
      const err = validateCoverageMetric(val, `telecom.${key}`);
      if (err) fail(err);
    }
    if (metrics.confirmedBusinessEdgeCount > 15) {
      warn(`telecom confirmed business count ${metrics.confirmedBusinessEdgeCount} exceeds Phase 5H cap 15`);
    }
  }

  if (sectorKey === 'chemical') {
    const listedTickers = new Set();
    const FORBIDDEN = new Set([
    ...
    for (const n of nodes) {
      if (n.type === 'listed_company' && n.id && !String(n.id).startsWith('krx:')) {
        fail(`chemical listed_company id must be krx: ${n.id}`);
      }
      if (FORBIDDEN.has(n.id)) fail(`chemical forbidden generic ID ${n.id}`);
      if (n.type === 'listed_company') {
        if (listedTickers.has(n.ticker)) fail(`chemical duplicate listed ticker ${n.ticker}`);
        listedTickers.add(n.ticker);
      }
    }
    for (const e of edges) {
      if (e.type === 'peer' && e.defaultHidden !== true) {
        fail(`chemical peer edge ${e.id} must be defaultHidden`);
      }
      if (e.type === 'cross_sector_reference') {
        if (e.excludesFromBusinessCoverage !== true) {
          warn(`chemical cross_sector_reference ${e.id} should set excludesFromBusinessCoverage`);
        }
        if (e.status === 'confirmed' || e.status === 'reported') {
          fail(`chemical cross_sector_reference ${e.id} must not be confirmed business`);
        }
      }
      if ((e.type === 'owns' || e.type === 'owns_stake_in') && e.status === 'confirmed') {
        if (e.stakePct == null || !e.asOf) fail(`chemical confirmed ownership ${e.id} needs stakePct/asOf`);
      }
    }
    const metrics = network.metrics || {};
    const cc = metrics.claimCoverage || {};
    for (const key of Object.keys(cc)) {
      ...
      const val = cc[key];
      if (!val || typeof val !== 'object') continue;
      const err = validateCoverageMetric(val, `chemical.${key}`);
      if (err) fail(err);
    }
  }
  ...
  if (sectorKey === 'travel') {
    ...
    for (const n of nodes) {
      if (n.type === 'listed_company' && n.id && !String(n.id).startsWith('krx:')) {
        fail(`travel listed_company id must be krx: ${n.id}`);
      }
      if (FORBIDDEN.has(n.id)) fail(`travel forbidden generic ID ${n.id}`);
      if (n.type === 'listed_company') {
        if (listedTickers.has(n.ticker)) fail(`travel duplicate listed ticker ${n.ticker}`);
        listedTickers.add(n.ticker);
      }
    }
    for (const e of edges) {
      if (e.type === 'peer' && e.defaultHidden !== true) {
        fail(`travel peer edge ${e.id} must be defaultHidden`);
      }
      if (e.type === 'cross_sector_reference') {
        if (e.excludesFromBusinessCoverage !== true) {
          warn(`travel cross_sector_reference ${e.id} should set excludesFromBusinessCoverage`);
        }
        if (e.status === 'confirmed' || e.status === 'reported') {
          fail(`travel cross_sector_reference ${e.id} must not be confirmed business`);
        }
      }
    }
    const metrics = network.metrics || {};
    ...
    for (const key of Object.keys(cc)) {
      ...
      const val = cc[key];
      if (!val || typeof val !== 'object') continue;
      const err = validateCoverageMetric(val, `travel.${key}`);
      if (err) fail(err);
    }
  }
